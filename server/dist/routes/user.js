...
router.post('/login', async (req, res) => {
    try {
        ...
        const user = await UserService_1.userService.findOrCreateByOpenid(openid, nickname, avatar);
        if (!user) {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const userInfo = await UserService_1.userService.getUserInfo(userId);
        if (!userInfo) {
